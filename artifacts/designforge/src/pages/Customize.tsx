...
export default function Customize() {
  ...
  useEffect(() => {
    if (template) {
      ...
      const hasCustomizations =
        Object.keys(customizations.texts).length > 0 ||
  ...
  const textFields =
    template?.customizableFields.filter((f) => f.type === "text") || [];
  const colorFields =
    template?.customizableFields.filter((f) => f.type === "color") || [];
  const fontFields =
    template?.customizableFields.filter((f) => f.type === "font") || [];
          ...
          <motion.div
            ...
            style={{
              backgroundColor:
                ...
                template?.customizableFields.find((f) => f.key === "background")
                  ?.defaultValue ||
                "var(--card)",
              color:
                customizations.colors.text ||
                template?.customizableFields.find((f) => f.key === "text")
                  ?.defaultValue ||
                "var(--foreground)",
              fontFamily:
                customizations.fonts.heading ||
                template?.customizableFields.find((f) => f.key === "heading")
                  ?.defaultValue ||
                "sans-serif",
            }}
          >
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div className="space-y-4">
                <h2
                  ...
                  style={{ color: customizations.colors.primary || "inherit" }}
                >
                  {customizations.texts.title ||
                    template?.customizableFields.find((f) => f.key === "title")
                      ?.defaultValue ||
                    template?.title}
                </h2>
                <p
                  ...
                  style={{
                    fontFamily: customizations.fonts.body || "sans-serif",
                  }}
                >
                  {customizations.texts.subtitle ||
                    template?.customizableFields.find(
                      (f) => f.key === "subtitle",
                    )?.defaultValue ||
              ...
              <div className="mt-auto glass p-6 rounded-xl border border-white/10">
                ...
                >
                  {customizations.texts.description ||
                    template?.customizableFields.find(
                      (f) => f.key === "description",
                    )?.defaultValue ||
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    {textFields.map((field) => (
                      <div key={field.key} className="space-y-2 mb-4">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCustomization("texts", field.key, e.target.value)
                          }
                    ...
                    {textFields.length === 0 && (
                      <p className="text-muted-foreground text-sm italic">
                        No text fields available for this template.
                      </p>
                    )}
                  </motion.div>
                </TabsContent>
                ...
                >
                  ...
                  >
                    {/* Luxury Color Picker */}
                    <div>
                      <h3 className="text-lg font-semibold mb-4">Luxury Color Palette</h3>
                      <LuxuryColorPicker
                        onSelectColor={(hex) => {
                          const primaryField = colorFields[0];
                          if (primaryField) {
                            setCustomization("colors", primaryField.key, hex);
                          }
                        }}
                        maxSelections={5}
                        displayMode="grid"
                    ...
                    {/* Additional manual color overrides */}
                    <div className="border-t pt-6">
                      <h3 className="text-lg font-semibold mb-4">Manual Color Adjustments</h3>
                            ...
                            <div className="flex gap-2 items-center">
                              <div className="relative w-10 h-10 rounded-md border border-border overflow-hidden shrink-0">
                                <input
                                  ...
                                  id={`color-${field.key}`}
                                  value={
                                    customizations.colors[field.key] || "#ffffff"
                                  }
                                  onChange={(e) =>
                                    setCustomization(
                                      "colors",
                      ...
                      {colorFields.length === 0 && (
                        <p className="text-muted-foreground text-sm italic">
                          No color fields available for this template.
                        </p>
                      )}
                    </div>
                  </motion.div>
                ...
                >
                  ...
                  >
                    {/* Luxury Font Picker */}
                    <div>
                      <h3 className="text-lg font-semibold mb-4">Premium Typefaces</h3>
                      <LuxuryFontPicker
                        onSelectFont={(fontFamily) => {
                          const headingField = fontFields[0];
                          if (headingField) {
                            setCustomization("fonts", headingField.key, fontFamily);
                          }
                        }}
                        maxSelections={5}
                        displayMode="list"
                    ...
                    {/* Additional font selector */}
                    <div className="border-t pt-6">
                      <h3 className="text-lg font-semibold mb-4">Font Options</h3>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {field.options?.map((font) => (
                              <button
                                key={font}
                                onClick={() =>
                                  setCustomization("fonts", field.key, font)
                                }
                      ...
                      {fontFields.length === 0 && (
                        <p className="text-muted-foreground text-sm italic">
                          No font fields available for this template.
                        </p>
                      )}
                    </div>
                  </motion.div>
        ...
        <div className="p-6 bg-muted/10 border-t border-border/50">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <p className="font-medium">
                {template?.deliveryTime || "2-3 days"}
              </p>
            </div>
          </div>
          ...
          >
            {submitRequest.isPending ? (
              ...
              <span className="flex items-center">
                <CheckCircle2 className="w-5 h-5 mr-2" /> Place Request
              </span>
            )}
          </Button>
        </div>
